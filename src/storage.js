 ...
 */
export function createSync({ onRemoteState, onStatus }) {
  let pending = null
  let saveTimer = null
  let inFlight = false
  ...
  return {
    ...
    bootstrap: async () => {
      ...
      try {
        ...
        if (meta.dirty) {
          // We have un-pushed local edits — keep local, push it up.
          pending = cached
          saveMeta({ updated_at: lastServerAt, dirty: true })
          scheduleFlush(50)
          return cached
        ...
        if (isEmptyState(server) && !isEmptyState(cached)) {
          // First-ever sync from a device that already had local data.
          pending = cached
          scheduleFlush(50)
          return cached
        }
        // Server wins on cold boot.
        writeCache(server)
