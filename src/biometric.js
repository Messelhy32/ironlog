...
// Local-only — the API still trusts the bearer token. Biometric just gates
// access to the stored token on each fresh app open.

const CRED_KEY = 'ironlog.v2.cred'
...
export async function enrollBiometric() {
  ...
  const cred = await navigator.credentials.create({
    publicKey: {
      ...
      user: { id: userId, name: 'iron-log', displayName: 'IRON LOG' },
      pubKeyCredParams: [
